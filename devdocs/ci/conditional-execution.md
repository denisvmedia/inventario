...
| `frontend_i18n`    | `frontend/src/**`, `i18next.config.ts`, `scripts/i18n-check.mjs`, `frontend/package*.json`                                                               |
...
| Backend only (`go/**`)               | All 4 `go-*` workflows + `frontend-codegen` (Go drives swagger.json) + `frontend-embed-smoke-test` + `docker.yml` + `e2e-tests-linux[chromium]`; Firefox + macOS-webkit + other frontend-* skip |
...
| Frontend codegen driver only (`frontend/scripts/codegen.mjs`) | `frontend-codegen`, and (via `frontend/**`) the rest of the `frontend-*` suite, `docker.yml`, full `e2e-tests` matrix                                                            |
