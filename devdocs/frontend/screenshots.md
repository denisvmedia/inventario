# Local screenshots of the frontend

How to spin up the React frontend (#1397) end-to-end on your machine and
capture screenshots of every key page. Useful for PR demos, design
reviews, or anyone asking "what can you actually see right now?".

The script we use is `e2e/screenshots.mjs`.
...
The Go binary embeds the React bundle at build time (via `//go:embed`
gated on the `with_frontend` build tag). The `dist/` directory must
...
- **`go:embed` build error: `pattern "all:dist": no matching files`.**
  You didn't run `make build-frontend` first. The `frontend/dist/`
  directory must exist for the embed to compile.
...
`e2e/screenshots.mjs`. Keep the filename prefix monotonic so the
