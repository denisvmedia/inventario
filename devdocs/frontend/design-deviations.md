...
- **Resolved**: 2026-05-23, PR #NNNN — back to 1:1; AI photo-scan now wired through the new backend service (issue #1720). `AiScanStep` ports the full four-phase state machine (`offer` / `scanning` / `review` / `error`) from the mock; the inert tracker line and `scanDisabledTitle` / `comingSoon` i18n keys are removed.
