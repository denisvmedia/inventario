 ...
 * return the env-var bundle that points the BE at it, for both providers.
 * The vars are merged into the BE spawn env below so the bootstrap layer
 * registers each provider with the stub-server URLs.
 *
...
async function maybeStartOAuthStub(): Promise<Record<string, string>> {
  ...
  return {
    ...
    INVENTARIO_RUN_OAUTH_GOOGLE_CLIENT_SECRET:
      process.env.INVENTARIO_RUN_OAUTH_GOOGLE_CLIENT_SECRET ??
      "stub-client-secret",
    ...
    INVENTARIO_RUN_OAUTH_STATE_KEY:
      process.env.INVENTARIO_RUN_OAUTH_STATE_KEY ??
      "e2e-stub-oauth-state-key-32-bytes!",
    ...
    // GitHub's leg (#1929). Four overrides rather than three: the profile
    // and the verified address come from separate endpoints. The bootstrap
    // layer refuses a partial set, so these move together or not at all.
    INVENTARIO_RUN_OAUTH_GITHUB_CLIENT_ID:
      process.env.INVENTARIO_RUN_OAUTH_GITHUB_CLIENT_ID ??
      "stub-github-client-id",
    INVENTARIO_RUN_OAUTH_GITHUB_CLIENT_SECRET:
      process.env.INVENTARIO_RUN_OAUTH_GITHUB_CLIENT_SECRET ??
      "stub-github-client-secret",
    INVENTARIO_RUN_OAUTH_GITHUB_AUTH_URL_OVERRIDE: `${stubURL}/gh/authorize`,
    INVENTARIO_RUN_OAUTH_GITHUB_TOKEN_URL_OVERRIDE: `${stubURL}/gh/token`,
    INVENTARIO_RUN_OAUTH_GITHUB_USER_URL_OVERRIDE: `${stubURL}/gh/user`,
    INVENTARIO_RUN_OAUTH_GITHUB_USER_EMAILS_URL_OVERRIDE: `${stubURL}/gh/user/emails`,
...
export async function startBackend(): Promise<void> {
  ...
  backendProcess = spawn(
    "go",
    ["run", "-tags", "with_frontend", "./cmd/inventario/...", "run"],
    {
      cwd: backendRoot,
      ...
      env: {
        ...
        INVENTARIO_RUN_AUTH_RATE_LIMIT_DISABLED:
          process.env.INVENTARIO_RUN_AUTH_RATE_LIMIT_DISABLED ?? "true",
        INVENTARIO_RUN_GLOBAL_RATE_LIMIT_DISABLED:
          process.env.INVENTARIO_RUN_GLOBAL_RATE_LIMIT_DISABLED ?? "true",
        ...
        INVENTARIO_RUN_MAX_GROUP_MEMBERSHIPS_PER_USER:
          process.env.INVENTARIO_RUN_MAX_GROUP_MEMBERSHIPS_PER_USER ?? "0",
        ...
        INVENTARIO_RUN_MEMORY_TENANT_SLUG:
          process.env.INVENTARIO_RUN_MEMORY_TENANT_SLUG ?? "test-org",
        INVENTARIO_RUN_MEMORY_TENANT_NAME:
          process.env.INVENTARIO_RUN_MEMORY_TENANT_NAME ?? "Test Organization",
        ...
        INVENTARIO_RUN_SUPPORT_EMAIL:
          process.env.INVENTARIO_RUN_SUPPORT_EMAIL ?? "support@e2e.test",
  ...
  while (retries < maxRetries) {
    try {
      ...
      if (retries === maxRetries) {
        throw new Error(
          "Backend server failed to start within the expected time",
        );
      }
    }
  }
...
export async function seedTenant(tenantSlug: string): Promise<void> {
  ...
  if (response.status !== 200) {
    throw new Error(
      `Failed to seed tenant '${tenantSlug}': ${response.statusText}`,
    );
  }
...
async function waitForFrontend(
  ...
  while (retries < maxRetries) {
    try {
      console.log(
        `Attempt ${retries + 1}/${maxRetries} to connect to frontend...`,
      );
      ...
      if (retries === maxRetries) {
        throw new Error(
          "Frontend server failed to start within the expected time",
        );
      }
    }
  }
...
export async function startStack(): Promise<void> {
  if (USE_PREBUILT) {
    ...
    console.log(
      `USE_PREBUILT=true — waiting for pre-started stack at ${BASE_URL}`,
    );
    await waitForBackend();
    await waitForFrontend();
    return;
