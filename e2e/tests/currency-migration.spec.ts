...
test.describe('Currency migration wizard (#1553)', () => {
  test('admin runs the wizard end-to-end and the preview surfaces totals', async ({
    ...
    try {
      ...
      await page.click(
        '[data-testid="migrate-currency-dialog"] [role="combobox"]',
      );
  ...
  test('migrate CTA is hidden for non-admins via the same-currency disabled state', async ({
    page,
  }) => {
    // Admin renders the page; the migrate CTA is enabled by default
    // and clicking it opens the wizard. We don't have a non-admin
    // fixture wired up here — this case asserts the CTA renders with
    // the expected attributes so a regression on the test-id wiring is
    // caught immediately.
    const auth = await readAuth(page);
    if (!auth.accessToken) {
      test.skip(true, 'no auth token captured — running without a seeded admin user');
    ...
    // Use the admin's existing default group (created by seeddata).
    await page.goto('/profile');
    // Drill into the first group's settings via the GroupSelector by
    // navigating to /groups/{id}/settings. We can't easily fish the
    // group id out of the UI here, so just hit the API and pick any
    // group the admin owns.
    ...
    // The admin sees the CTA with no in-flight migration → enabled.
