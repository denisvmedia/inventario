...
func TestAreaRegistry_Create(t *testing.T) {
	...
	createdArea, err := areaRegistry.Create(area)
	c.Assert(err, qt.IsNil)
...
func TestAreaRegistry_Create_Validation(t *testing.T) {
	...
	_, err := areaRegistry.Create(area)
...
func TestAreaRegistry_Commodities(t *testing.T) {
	...
	err = areaRegistry.AddCommodity(createdArea.ID, "commodity1")
	c.Assert(err, qt.IsNil)
	err = areaRegistry.AddCommodity(createdArea.ID, "commodity2")
	c.Assert(err, qt.IsNil)

	// Get the commodities of the area
	commodities, err := areaRegistry.GetCommodities(createdArea.ID)
	c.Assert(err, qt.IsNil)
	c.Assert(commodities, qt.Contains, "commodity1")
	c.Assert(commodities, qt.Contains, "commodity2")

	// Delete a commodity from the area
	err = areaRegistry.DeleteCommodity(createdArea.ID, "commodity1")
	c.Assert(err, qt.IsNil)
...
func TestAreaRegistry_Delete(t *testing.T) {
	...
	_, err = areaRegistry.Get(createdArea.ID)
	c.Assert(err, qt.IsNil)

	// Delete a non-existing area from the registry
	err = areaRegistry.Delete("non-existing")
	c.Assert(err, qt.ErrorIs, registry.ErrNotFound)

	// Delete the area from the registry
	err = areaRegistry.Delete(createdArea.ID)
	c.Assert(err, qt.IsNil)
