...
//   - Reader: io.Reader
//   - ReaderOptions.BeforeRead: afero.File
...
func (b *bucket) forKey(key string) (string, os.FileInfo, *xattrs, error) {
	...
	xa, err := getAttrs(b.opts.FS, path)
	if err != nil {
		return "", nil, nil, err
	}
...
func createTemp(path string, opts *Options) (afero.File, error) {
	...
	for {
		...
		f, err := opts.FS.OpenFile(name, os.O_RDWR|os.O_CREATE|os.O_EXCL, 0666)
		if os.IsExist(err) {
			if try++; try < 10000 {
				continue
			}
...
func (b *bucket) NewTypedWriter(ctx context.Context, key string, contentType string, opts *driver.WriterOptions) (driver.Writer, error) {
	...
	if err := b.opts.FS.MkdirAll(filepath.Dir(path), os.FileMode(0777)); err != nil {
		return nil, err
	}
	f, err := createTemp(path, b.opts)
	if err != nil {
		return nil, err
	}
...
func (w *writerWithSidecar) Close() error {
	...
	if err := setAttrs(w.opts.FS, w.path, w.attrs); err != nil {
		return err
	}
...
//
// Embedding afero.File allows the likes of io.Copy to use optimizations.,
...
func (w *writer) Close() error {
	...
	defer w.opts.FS.Remove(tempname)
...
func (b *bucket) Delete(ctx context.Context, key string) error {
	...
	err = b.opts.FS.Remove(path)
	if err != nil {
		return err
	}
	if err = b.opts.FS.Remove(path + attrsExt); err != nil && !os.IsNotExist(err) {
		return err
	}
	return nil
}
