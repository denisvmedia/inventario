...
// it uses JSON format.
func setAttrs(fs afero.Fs, path string, xa xattrs) error {
	f, err := fs.Create(path + attrsExt)
	if err != nil {
		return err
	}
...
// such .attrs file.
func getAttrs(fs afero.Fs, path string) (xattrs, error) {
	f, err := fs.Open(path + attrsExt)
	if err != nil {
