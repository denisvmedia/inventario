...
> [#1865](https://github.com/denisvmedia/inventario/issues/1865).
...
Both ArgoCD ApplicationSets use a GitHub App to talk to the GitHub API:
- the PR generator (`inventario-pr-previews`) lists open PRs on
  `denisvmedia/inventario` (so it knows which PRs are labeled `preview`),
  and reads repo contents at each PR's head commit so the repo-server can
  `git clone` the chart;
- the git-`files` generator (`inventario-master-preview` /
  `inventario-longevity-preview`) reads `infra/argocd/master-image.json`
  off the CI-maintained `argocd/master-pin` branch so the templated
  `image.tag` flips on every master push (#1955 — scmProvider can't be
  used because `denisvmedia` is a user, not an org).

Both reuse the same `github-app-creds` Secret (repo-creds format created
by `apply-secrets.sh` from the sops bundle).
...
8. Installs Velero via helm + the matching CLI — ONLY when the `velero.*`
   keys are filled in the sops bundle (#1865). Creates the R2
   `cloud-credentials` + kopia `velero-repo-credentials` Secrets and the daily
   backup Schedule. Skipped (with a warning) on a bundle without those keys.
9. Materializes Kubernetes Secrets from the sops bundle (the `inventario-admin`
   seed for both `inv-vcl01-master` and `inv-vcl01-longevity`, the GitHub App
   repo-creds, the TS operator OAuth).
10. Applies the cluster-extras manifests (`ts-orphan-cleanup` CronJob, plus
    the tailnet Ingress that exposes the ArgoCD UI at
    `argocd-inv-vcl01.<tailnet>.ts.net`).
11. Applies the ArgoCD `AppProject` and three `ApplicationSet`s — the master
    preview (`inventario-master-preview` from #1885), the persistent longevity
    env (`inventario-longevity-preview` from #1865), and the PR previews
    (`inventario-pr-previews`).
12. Copies the kubeconfig back to `~/.kube/inv-vcl01.config` on your laptop,
...
# 4. The ApplicationSets exist.
KUBECONFIG=~/.kube/inv-vcl01.config kubectl -n argocd get appset
# expect: "inventario-master-preview" + "inventario-longevity-preview" +
#         "inventario-pr-previews"

# 5. The master + longevity Applications exist.
KUBECONFIG=~/.kube/inv-vcl01.config kubectl -n argocd get application inv-vcl01-master inv-vcl01-longevity
# expect: STATUS becomes Synced/Healthy within ~5 min (waits for the first
#         sha-<7> image build of the current master HEAD to finish on GHCR)
...
# 7. (If you filled velero.* in the bundle) Velero is up and the backend is
#    reachable. The first scheduled backup runs at 02:00 UTC; trigger one now
#    to confirm end-to-end (see §6 "Disaster recovery").
KUBECONFIG=~/.kube/inv-vcl01.config kubectl -n velero get pods
# expect: velero (1/1) + node-agent DaemonSet pod(s) Running
KUBECONFIG=~/.kube/inv-vcl01.config kubectl -n velero get backupstoragelocation
# expect: PHASE Available (Unavailable → check creds/endpoint, see §6)
```
...
`https://inv-vcl01-master.<your-tailnet-name>.ts.net/`. The Application is
generated by `ApplicationSet inventario-master-preview` using a git-`files`
generator (#1955). It does **not** use the `scmProvider` generator #1885
shipped: `scmProvider.github` only lists *organization* repos
(`GET /orgs/denisvmedia/repos` → 404, because `denisvmedia` is a user
account), so that generator produced no Application and the master preview
silently vanished from ArgoCD. Instead, on each master push `docker.yml`
force-pushes the head SHA + `sha-<7>` tag to a one-file orphan branch
`argocd/master-pin` (`infra/argocd/master-image.json`); the generator reads
that file and threads `{{ .imageTag }}` into `image.tag` and `{{ .sha }}`
into the chart revision. Every commit therefore produces a real manifest
diff, ArgoCD syncs, and the per-commit immutable image is pulled.
End-to-end: master push → docker.yml builds `sha-<7>` + repins
`argocd/master-pin` (~3-5 min) → ApplicationSet picks up the new pin on its
next 60 s poll → pods roll. No manual `rollout restart` needed. The
...
The two should match within a few minutes of a master push (the lag is the
docker.yml build time + ArgoCD's 60 s poll cadence). If the deployed SHA
falls behind by more than ~10 minutes, check the `Docker image` workflow on
the [Actions tab](https://github.com/denisvmedia/inventario/actions/workflows/docker.yml)
for a failed build.

> [!IMPORTANT]
> **If the master Application is missing from ArgoCD.** Two causes, both a
> consequence of the #1885 → #1955 history:
>
> 1. **The `inventario-master-preview` ApplicationSet isn't registered.** A
>    VM bootstrapped before #1885 only has the PR ApplicationSet (and a
>    long-gone static `Application/inv-vcl01-master` that left its namespace
>    orphaned). Re-run bootstrap to register the master + longevity
>    ApplicationSets:
>    ```bash
>    make -C infra bootstrap VM=user@host
>    # or, to apply just this one:
>    sed "s/<TAILNET>/$TAILNET_NAME/g" infra/argocd/applicationset-master.yaml \
>      | KUBECONFIG=~/.kube/inv-vcl01.config kubectl apply -f -
>    ```
> 2. **The `argocd/master-pin` branch doesn't exist yet.** The git-`files`
>    generator has nothing to read until the first master build after #1955
>    runs the `pin-master-image` job. Push to (or re-run CI on) master once;
>    the generator then produces `inv-vcl01-master` on its next poll and
>    reconciles the existing namespace via `selfHeal`.
>
> The pre-#1955 `scmProvider` generator never worked here at all — it lists
> *org* repos, and `denisvmedia` is a user account (`GET /orgs/.../repos`
> → 404). See #1884 for the broader in-place upgrade story.

### longevity deployment (persistent + backed up)

`Application inv-vcl01-longevity` (generated by `ApplicationSet
inventario-longevity-preview`, #1865) is a second master-tracking deployment
reachable at `https://inv-vcl01-longevity.<your-tailnet-name>.ts.net/`. It
rolls forward on every master push exactly like `inv-vcl01-master`, with ONE
difference: its demo Postgres + MinIO keep their data in PersistentVolumeClaims
instead of emptyDir. So the seeded/accumulated data **survives** pod restarts
and rolling deploys, and is captured nightly by Velero (next section).

This is the env to point at when you want to see whether a migration preserves
real data across continuous deployment — the "longevity" property the
ephemeral master/PR previews deliberately don't have. The trade-off is disk: it
adds two PVCs (5Gi Postgres + 10Gi MinIO) to the VM, so size the disk
accordingly (the Phase-1 ~40 GiB target gets tighter with both master and
longevity running). Sizing lives in
[`infra/helm-overlays/longevity-base.values.yaml`](./helm-overlays/longevity-base.values.yaml).

```bash
KUBECONFIG=~/.kube/inv-vcl01.config kubectl -n inv-vcl01-longevity get pods,pvc
```
...
| `make -C infra restore-longevity VM=user@host` | Interactive Velero restore of `inv-vcl01-longevity` from an R2 backup (#1865). Lists backups, prompts which, freezes ArgoCD, deletes + restores the namespace. See §6. |
...
# expect: inv-vcl01-master + inv-vcl01-longevity + any inv-vcl01-prN matching
...
What is NOT preserved by `make recover` alone:

- **Per-preview (master + PR) Postgres / Redis / MinIO data.** By design —
  those previews are ephemeral. The setup Job re-seeds on first install.
- **The longevity namespace's DATA.** `recover` re-creates
  `inv-vcl01-longevity` with fresh, re-seeded PVCs. To bring its actual data
  back, run `make -C infra restore-longevity` afterwards (step 6 above) — that
  restores the last Velero backup from R2. This is the one stateful workload
  the preview-env keeps durable.

> [!IMPORTANT]
> The single off-VM dependency for the system itself is your age private key.
> If Velero is configured, `velero.encryption_key` (the kopia repo password)
> is a SECOND off-VM-critical secret — without it the backups in R2 can't be
> decrypted on a fresh VM. Both live in the sops bundle, but back BOTH up in
> your password manager: lose the age key and the bundle is a brick; rotate or
> lose `velero.encryption_key` and the backup history is unrecoverable.

### Backups & restoring the longevity data (Velero → Cloudflare R2)

When the `velero.*` keys are filled in the sops bundle, `bootstrap` installs
Velero with a daily Schedule that File-System-Backs-up (kopia) every PVC in
`inv-vcl01-longevity` to a Cloudflare R2 bucket at **02:00 UTC, retained 30
days**. The kopia repo is encrypted with `velero.encryption_key`, so the data
at rest in R2 is unreadable without it.

```bash
# List backups (run on the VM, where the velero CLI + kubeconfig live):
ssh user@vm-host 'sudo env KUBECONFIG=/var/lib/vcluster/kubeconfig.yaml velero backup get'

# Trigger an on-demand backup now (don't wait for 02:00 UTC) — useful right
# after first bootstrap to confirm the pipeline end-to-end:
ssh user@vm-host 'sudo env KUBECONFIG=/var/lib/vcluster/kubeconfig.yaml \
  velero backup create longevity-adhoc-$(date +%s) \
  --include-namespaces inv-vcl01-longevity --default-volumes-to-fs-backup --wait'
```

Restoring (deliberate ns loss, corruption, or post-`recover` data recovery):

```bash
make -C infra restore-longevity VM=user@vm-host
```

The target is interactive: it lists backups, prompts which one, then **freezes
ArgoCD** (scales the application + applicationset controllers to 0 cluster-wide
for ~2-3 min), deletes `inv-vcl01-longevity` for a clean target, runs `velero
restore --wait`, and resumes ArgoCD. The freeze is required because Velero FSB
restores data into freshly-created PVCs — if ArgoCD's `selfHeal` recreated the
namespace with empty PVCs first, Velero would skip them and you'd restore
...
> kopia password and a fresh-VM restore will fail to decrypt. Set it BEFORE
> the first backup and never change it — see
> [SECRETS.md §"Never rotate velero.encryption_key"](./SECRETS.md#never-rotate-veleroencryption_key).

Backend not `Available`, or restore won't decrypt? See the Velero bullets in
[SECRETS.md §Troubleshooting](./SECRETS.md#troubleshooting).
...
Each has its own dedicated issue:

- **[#1865](https://github.com/denisvmedia/inventario/issues/1865) Velero
  backup/restore — ✅ implemented.** Daily kopia File System Backup of the
  persistent `inv-vcl01-longevity` namespace to Cloudflare R2, 30-day
  retention, with `make restore-longevity` as the restore flow. Enable it by
  filling the `velero.*` keys in the sops bundle. Full runbook in
  [§6 "Disaster recovery"](#6-disaster-recovery).
...
(in-place ArgoCD upgrades with migrations).
...
infra/
├── Makefile                  preflight bootstrap upgrade recover destroy restore-longevity status logs shell
├── README.md                 this file — the start-to-finish guide
├── SECRETS.md                credentials walkthrough (age, GH App, TS OAuth, R2, DR)
├── argocd/                          ArgoCD manifests applied by bootstrap.sh
│   ├── appproject.yaml                AppProject scoping previews/master/longevity to inv-vcl01-* ns
│   ├── applicationset-master.yaml     ApplicationSet SCM-provider generator (master branch — #1885)
│   ├── applicationset-longevity.yaml  ApplicationSet for the persistent, Velero-backed env (#1865)
...
│   └── longevity-base.values.yaml  layered over preview-base for inv-vcl01-longevity: PVC persistence (#1865)
└── vm/
    ├── bootstrap.sh          laptop orchestrator (ssh, sops, upload, apply)
    ├── vm-install.sh         remote installer (tailscale, vcluster, TS-op, ArgoCD, Velero)
    ├── destroy.sh            remote teardown (vcluster + data; tailnet preserved)
    ├── helm-values/          static chart overlays vm-install.sh layers secrets on top of
    │   ├── tailscale-operator.yaml  TS operator static values (OAuth layered at install)
    │   └── velero.yaml              Velero static values: plugin, node-agent, schedule (#1865)
    ├── cluster-extras/
    ...
    │   └── restore-longevity.sh   interactive Velero restore of inv-vcl01-longevity (#1865)
    └── secrets/
        ├── .gitignore
