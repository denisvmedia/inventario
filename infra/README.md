...
10. Applies the ArgoCD `AppProject` and two `ApplicationSet`s — one for the
    master preview (`inventario-master-preview` from #1885) and one for the
    PR previews (`inventario-pr-previews`).
...
# 4. Both ApplicationSets exist.
KUBECONFIG=~/.kube/inv-vcl01.config kubectl -n argocd get appset
# expect: "inventario-master-preview" + "inventario-pr-previews"

# 5. The master Application exists (generated by inventario-master-preview).
