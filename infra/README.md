...
- the PR generator (`inventario-pr-previews`) lists open PRs on
  ...
  `git clone` the chart;
- the SCM-provider generator (`inventario-master-preview`, #1885) reads
  the `master` branch's current head SHA so the templated `image.tag`
  flips on every push.
...
generated by `ApplicationSet inventario-master-preview` (#1885) using the
SCM-provider generator: it polls GitHub for `master`'s head SHA every 60s
and threads it into `image.tag: sha-<7>`. Every commit therefore produces a
real manifest diff, ArgoCD syncs, and the per-commit immutable image is
pulled. End-to-end: master push → docker.yml builds `sha-<7>` (~3-5 min)
→ ApplicationSet picks up the new SHA on its next poll → pods roll. No
manual `rollout restart` needed.
...
> [!IMPORTANT]
> **Migrating from the pre-#1885 static Application.** If you bootstrapped
> a VM before #1885 landed, an existing static `Application/inv-vcl01-master`
> sits in the `argocd` namespace without an owner-ref to the new
> ApplicationSet. ArgoCD won't take it over automatically. Delete it once
> with `--cascade=orphan` (keeps the namespace + workloads in place) before
> the next `make bootstrap`:
> ```bash
> KUBECONFIG=~/.kube/inv-vcl01.config kubectl -n argocd \
>   delete application inv-vcl01-master --cascade=orphan
> make -C infra bootstrap VM=user@host
> ```
> The ApplicationSet then re-creates `inv-vcl01-master` with the right
> ownership and reconciles the existing namespace via `selfHeal`. See
> #1884 for the broader in-place upgrade story.
