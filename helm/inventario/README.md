...
- **External Postgres still requires bootstrap.** With `demo.postgresql.enabled=false`, the bootstrap Job at wave -5 connects to the external Postgres using `setupJob.bootstrap.superuserDsn` (or the `SETUP_SUPERUSER_DSN` key in `secrets.existingSecret`). If your platform creates DB roles out-of-band, set `setupJob.bootstrap.enabled=false` — the chart will skip the bootstrap container; the operator is responsible for ensuring `inventario_migrator` exists before the Deployment starts, **and that the `pg_trgm` and `pgcrypto` extensions are installed**. Bootstrap is the step that creates them, and the migrator role cannot: it has `CREATE` on the schema, not on the database. `pg_trgm` backs the trigram indexes; `pgcrypto` is needed because a full replay reaches migration `1772465439`, which issues `CREATE EXTENSION IF NOT EXISTS pgcrypto` — a no-op only where the extension already exists. `inventario db migrate up` checks both up front and names what is missing, rather than failing partway through the chain.
