...
- **Routing**: `frontend/` uses TanStack Router (see `frontend/src/app/router.tsx`). Replace the mock's `setView("x")` with a router navigation — but keep the *callback shape* the view uses (`onNavigate?: (view: string) => void`). This isn't a deviation; it's translation.
...
- **Inline `style={{ color: ... }}` for color or spacing.** Tailwind utilities only. The single legitimate inline style is when a *data value* drives geometry (e.g. `width: ${pct}%` on the dashboard breakdown bar, see `views/DashboardView.tsx:236`).
