...
const commodityService = {
  ...
  async uploadImage(id: string, file: File, _onProgress?: (number, number, string) => void): Promise<any> {
  ...
  async uploadImages(id: string, files: File[], _onProgress?: (number, number, string) => void): Promise<any[]> {
  ...
  async uploadImageWithRetry(id: string, file: File, _onProgress?: (number, number, string) => void, maxRetries: number = 10): Promise<any> {
  ...
  async uploadManual(id: string, file: File, _onProgress?: (number, number, string) => void): Promise<any> {
    const formData = new FormData()
    formData.append('file', file)
  ...
  // - Provide per-file progress callbacks instead of aggregate progress
  // - Remove completed files from UI automatically
  async uploadManuals(id: string, files: File[], _onProgress?: (number, number, string) => void): Promise<any[]> {
    // Get max concurrent uploads from API
    const uploadSlotService = (await import('./uploadSlotService')).default
  ...
  async uploadManualWithRetry(id: string, file: File, _onProgress?: (number, number, string) => void, maxRetries: number = 10): Promise<any> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
  ...
  async uploadInvoice(id: string, file: File, _onProgress?: (number, number, string) => void): Promise<any> {
    const formData = new FormData()
    formData.append('file', file)
  ...
  // - Provide per-file progress callbacks instead of aggregate progress
  // - Remove completed files from UI automatically
  async uploadInvoices(id: string, files: File[], _onProgress?: (number, number, string) => void): Promise<any[]> {
    // Get max concurrent uploads from API
    const uploadSlotService = (await import('./uploadSlotService')).default
    const statusResponse = await uploadSlotService.getUploadStatus('document_upload')
    const maxConcurrent = statusResponse.data.attributes.max_uploads
  ...
  async uploadInvoiceWithRetry(id: string, file: File, _onProgress?: (number, number, string) => void, maxRetries: number = 10): Promise<any> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
