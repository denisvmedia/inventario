...
func TestColumnBuilder_FluentChaining(t *testing.T) {
	...
	result7 := columnBuilder.DefaultFunction("NOW()")
...
func TestColumnBuilder_MultipleColumns(t *testing.T) {
	...
	table := builder.NewTable("users").
		...
		Column("created_at", "TIMESTAMP").NotNull().DefaultFunction("NOW()").End().
		Column("updated_at", "TIMESTAMP").DefaultFunction("NOW()").End()
	...
	c.Assert(createdAtCol.Default.Function, qt.Equals, "NOW()")
}
