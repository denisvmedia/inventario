...
func TestCurrency_Validate(t *testing.T) {
	...
	t.Run("invalid currency code", func(t *testing.T) {
		c.Assert(models.Currency("").Validate(), qt.Not(qt.IsNil))
		c.Assert(models.Currency("FOO").Validate(), qt.Not(qt.IsNil))
		c.Assert(models.Currency("US").Validate(), qt.Not(qt.IsNil))
		c.Assert(models.Currency("USDD").Validate(), qt.Not(qt.IsNil))
	})
}
