...
type SchemaWriter interface {
	...
	CheckSchemaExists(result *migratorlib.PackageParseResult) ([]string, error)
}
...
func (w *PostgreSQLWriter) WriteSchema(result *migratorlib.PackageParseResult) error {
	// Start transaction
...
func (w *PostgreSQLWriter) DropSchema(result *migratorlib.PackageParseResult) error {
...
// WriteSchema writes the complete schema to the database
func (w *MySQLWriter) WriteSchema(result *migratorlib.PackageParseResult) error {
	// Start transaction
	if err := w.BeginTransaction(); err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
	}
	...
	statements := result.GetOrderedCreateStatements("mysql")
	for i, statement := range statements {
		fmt.Printf("Creating table %d/%d...\n", i+1, len(statements))
...
// CheckSchemaExists checks if any tables from the schema already exist
func (w *MySQLWriter) CheckSchemaExists(result *migratorlib.PackageParseResult) ([]string, error) {
	var existingTables []string
