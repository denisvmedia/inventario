...
func TestGenerator_New(t *testing.T) {
	...
	c.Assert(generator.GetDialectName(), qt.Equals, "mysql")
}
...
func TestGenerator_GenerateCreateTable_BasicTable(t *testing.T) {
	...
	c.Assert(result, qt.Contains, "-- MYSQL TABLE: users (User accounts table) --")
...
func TestGenerator_GenerateCreateTable_WithInlineEnums(t *testing.T) {
	...
	// MySQL should create inline ENUM types
...
func TestGenerator_GenerateCreateTable_WithTypeOverrides(t *testing.T) {
	...
	fields := []meta.SchemaField{
		...
		},
		{
			...
			Overrides: map[string]map[string]string{
				"mysql": {
					"type": "JSON", // MySQL-specific override
				},
			},
		},
	}
	...
	c.Assert(result, qt.Contains, "data JSON")         // Should use the MySQL override
...
func TestGenerator_GenerateCreateTable_WithTableOverrides(t *testing.T) {
	...
	c.Assert(result, qt.Contains, "-- MYSQL TABLE: users (Legacy user table) --")
	c.Assert(result, qt.Contains, "ENGINE=MyISAM")
...
func TestGenerator_GenerateCreateTable_WithCheckOverrides(t *testing.T) {
	...
	fields := []meta.SchemaField{
		{
			...
			Overrides: map[string]map[string]string{
				"mysql": {
					"check": "price >= 0", // MySQL-specific override
				},
			},
		},
		{
			...
			Name:       "quantity",
			Type:       "INTEGER",
			// No default check, only platform-specific
			Overrides: map[string]map[string]string{
				"mysql": {
					"check": "quantity > 0", // MySQL-specific check
				},
			},
		},
	}
	...
	c.Assert(result, qt.Contains, "price DECIMAL(10,2) NOT NULL CHECK (price >= 0)") // Should use MySQL override
	c.Assert(result, qt.Not(qt.Contains), "CHECK (price > 0)")                       // Should not use default
	c.Assert(result, qt.Contains, "quantity INTEGER NOT NULL CHECK (quantity > 0)")  // Should use MySQL-specific check
}
...
func TestGenerator_GenerateAlterStatements(t *testing.T) {
	...
	// MySQL should generate appropriate ALTER statements
