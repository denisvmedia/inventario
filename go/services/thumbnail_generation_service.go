...
func (s *ThumbnailGenerationService) RequestThumbnailGeneration(ctx context.Context, fileID string) (*models.ThumbnailGenerationJob, error) {
	...
	if err != nil {
		...
		return nil, errxtrace.Wrap("failed to check rate limit", err)
	}

	// Check if a job already exists for this file
	existingJob, err := jobRegistry.GetJobByFileID(ctx, fileID)
	if err != nil && !errors.Is(err, registry.ErrNotFound) {
		return nil, errxtrace.Wrap("failed to check for existing thumbnail job", err)
	}
...
func (s *ThumbnailGenerationService) ProcessThumbnailGeneration(ctx context.Context, jobID string) error {
	...
	if err != nil {
		return errxtrace.Wrap("failed to get thumbnail generation job", err)
	}
	...
	if err != nil {
		return errxtrace.Wrap("failed to get user concurrency slots", err)
	}
	...
	if err != nil {
		return errxtrace.Wrap("failed to update job status to processing", err)
	}
	...
	if err != nil {
		...
		return errxtrace.Wrap("failed to get file for thumbnail generation", err)
	}
	...
	if err != nil {
		...
		return errxtrace.Wrap("failed to generate thumbnails", err)
	}
	...
	if err != nil {
		...
		return errxtrace.Wrap("failed to mark job as completed", err)
	}
...
func (s *ThumbnailGenerationService) GetPendingJobs(ctx context.Context, limit int) ([]*models.ThumbnailGenerationJob, error) {
	...
	if err != nil {
		return nil, errxtrace.Wrap("failed to get pending thumbnail jobs", err)
	}
...
func (s *ThumbnailGenerationService) GetJobByFileID(ctx context.Context, fileID string) (*models.ThumbnailGenerationJob, error) {
	...
	if err != nil {
		return nil, errxtrace.Wrap("failed to get thumbnail job by file ID", err)
	}
...
func (s *ThumbnailGenerationService) CleanupCompletedJobs(ctx context.Context, olderThan time.Duration) error {
	...
	if err != nil {
		return errxtrace.Wrap("failed to cleanup completed thumbnail jobs", err)
	}
