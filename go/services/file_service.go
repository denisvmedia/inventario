...
func (s *FileService) GenerateThumbnails(ctx context.Context, file *models.FileEntity) error {
	...
	if err != nil {
		return stacktrace.Wrap("failed to open bucket", err)
	}
	defer b.Close()
	...
	if err != nil {
		return stacktrace.Wrap("failed to open original image", err)
	}
	defer reader.Close()
	...
	if err != nil {
		return stacktrace.Wrap("failed to decode image", err)
	}
	...
	for sizeName, maxSize := range s.thumbnailSizes {
		...
		if err != nil {
			return stacktrace.Wrap("failed to create thumbnail writer", err)
		}
		...
		writer.Close()
		if err != nil {
			return stacktrace.Wrap("failed to encode thumbnail", err)
		}
	}
...
func (s *FileService) DeleteFileWithPhysical(ctx context.Context, fileID string) error {
	...
	if err != nil {
		return stacktrace.Wrap("failed to create file registry", err)
	}

	// Get the file entity first
	file, err := fileReg.Get(ctx, fileID)
	if err != nil {
		return stacktrace.Wrap("failed to get file entity", err)
	}
	...
	if file.File != nil && file.File.OriginalPath != "" {
		if err := s.deletePhysicalFileAndThumbnails(ctx, fileID, file.File.OriginalPath, file.File.MIMEType); err != nil {
			return stacktrace.Wrap("failed to delete physical file and thumbnails", err)
		}
	}
	...
	if err != nil {
		return stacktrace.Wrap("failed to delete file entity", err)
	}
...
func (s *FileService) deletePhysicalFileAndThumbnails(ctx context.Context, fileID, filePath, mimeType string) error {
	...
	if err := s.deletePhysicalFile(ctx, filePath); err != nil {
		return stacktrace.Wrap("failed to delete original file", err)
	}
...
func (s *FileService) deletePhysicalFile(ctx context.Context, filePath string) error {
	...
	if err != nil {
		return stacktrace.Wrap("failed to check if file exists", err)
	}
	...
	if err != nil {
		return stacktrace.Wrap("failed to delete file", err)
	}
...
func (s *FileService) DeleteLinkedFiles(ctx context.Context, entityType, entityID string) error {
	...
	if err != nil {
		return stacktrace.Wrap("failed to get linked files", err)
	}
	...
	for _, file := range files {
		...
		if err != nil && !errors.Is(err, registry.ErrNotFound) {
			return stacktrace.Wrap("failed to delete linked file", err)
		}
	}
...
func (s *FileService) ThumbnailExists(ctx context.Context, fileID, size string) (bool, error) {
	...
	if size != "small" && size != "medium" {
		return false, stacktrace.Classify(ErrInvalidThumbnailSize, errx.Attrs("size", size))
	}
	...
	b, err := blob.OpenBucket(ctx, s.uploadLocation)
	if err != nil {
		return false, stacktrace.Wrap("failed to open bucket", err)
	}
	defer b.Close()

	exists, err := b.Exists(ctx, thumbnailPath)
	if err != nil {
		return false, stacktrace.Wrap("failed to check thumbnail existence", err, errx.Attrs("thumbnail_path", thumbnailPath))
	}
