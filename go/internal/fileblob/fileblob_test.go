...
func TestSignedURLReturnsUnimplementedWithNoURLSigner(t *testing.T) {
	...
	dir, err := afero.TempDir(fs, "", "fileblob")
	if err != nil {
		t.Fatal(err)
	}
	defer fs.RemoveAll(dir)
	b, err := OpenBucket(dir, &Options{
		FS: fs,
	})
	if err != nil {
		t.Fatal(err)
	...
	if gcerrors.Code(gotErr) != gcerrors.Unknown { // Should have been Unimplemented, but gerr is not exported and when we use a copy of it, it cannot be equal to gcerrors.Unimplemented.
