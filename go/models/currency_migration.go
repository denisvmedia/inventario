...
//
//ptah:schema:rls:enable table="currency_migrations" comment="Enable RLS for multi-tenant currency migration isolation"
...
//ptah:schema:rls:policy name="currency_migration_background_worker_access" table="currency_migrations" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows background workers to claim, advance, and recover currency migration rows"
...
//
//ptah:schema:table name="currency_migrations"
type CurrencyMigration struct {
	...
	//ptah:schema:field name="from_currency" type="TEXT" not_null="true"
	...
	//ptah:schema:field name="to_currency" type="TEXT" not_null="true"
	...
	//ptah:schema:field name="exchange_rate" type="DECIMAL(20,10)" not_null="true"
	...
	//ptah:schema:field name="commodity_count" type="INTEGER" not_null="true" default="0"
	...
	// computed them.
	//ptah:schema:field name="total_before" type="DECIMAL(20,2)"
	TotalBefore *decimal.Decimal `json:"total_before,omitempty" db:"total_before" userinput:"false"`
	//ptah:schema:field name="total_after" type="DECIMAL(20,2)"
	...
	//ptah:schema:field name="preview_token" type="TEXT"
	...
	//ptah:schema:field name="preview_expires_at" type="TIMESTAMP"
	...
	//ptah:schema:field name="started_at" type="TIMESTAMP"
	...
	//ptah:schema:field name="completed_at" type="TIMESTAMP"
...
type CurrencyMigrationIndexes struct {
	...
	//ptah:schema:index name="idx_currency_migrations_uuid" fields="uuid" unique="true" table="currency_migrations"
	_ int

	// Composite index for tenant + group lookups (history list).
	//ptah:schema:index name="idx_currency_migrations_tenant_group" fields="tenant_id,group_id" table="currency_migrations"
	_ int

	// Composite index for worker queries (group + status, e.g. ClaimNextPending).
	//ptah:schema:index name="idx_currency_migrations_group_status" fields="group_id,status" table="currency_migrations"
	_ int
	...
	//ptah:schema:index name="idx_currency_migrations_group_completed" fields="group_id,completed_at" condition="status = 'completed'" table="currency_migrations"
	_ int
	...
	//ptah:schema:index name="idx_currency_migrations_group_in_flight" fields="group_id" unique="true" condition="status IN ('pending', 'running')" table="currency_migrations"
	_ int
}
...
//
//ptah:schema:rls:enable table="currency_migration_audit_rows" comment="Enable RLS for currency migration audit rows"
...
//ptah:schema:rls:policy name="currency_migration_audit_background_worker_access" table="currency_migration_audit_rows" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows the background worker to insert audit rows in TX2"

//ptah:schema:table name="currency_migration_audit_rows"
type CurrencyMigrationAuditRow struct {
	...
	//ptah:schema:field name="migration_id" type="TEXT" not_null="true" foreign="currency_migrations(id)" foreign_key_name="fk_currency_migration_audit_migration" on_delete="CASCADE"
	...
	//ptah:schema:field name="commodity_id" type="TEXT" foreign="commodities(id)" foreign_key_name="fk_currency_migration_audit_commodity" on_delete="SET NULL"
	...
	//ptah:schema:field name="acquisition_filled_in_this_run" type="BOOLEAN" not_null="true" default="false"
...
type CurrencyMigrationAuditRowIndexes struct {
	//ptah:schema:index name="idx_currency_migration_audit_uuid" fields="uuid" unique="true" table="currency_migration_audit_rows"
	_ int

	//ptah:schema:index name="idx_currency_migration_audit_migration" fields="migration_id" table="currency_migration_audit_rows"
	_ int

	//ptah:schema:index name="idx_currency_migration_audit_commodity" fields="commodity_id" table="currency_migration_audit_rows"
	_ int

	//ptah:schema:index name="idx_currency_migration_audit_tenant_group" fields="tenant_id,group_id" table="currency_migration_audit_rows"
	_ int
}
