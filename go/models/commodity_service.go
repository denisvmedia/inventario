...
//
//ptah:schema:rls:enable table="commodity_services" comment="Enable RLS for multi-tenant commodity service isolation"
...
//ptah:schema:rls:policy name="commodity_service_background_worker_access" table="commodity_services" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows background workers to access all commodity services for processing"
...
type CommodityService struct {
	...
	//ptah:schema:field name="commodity_id" type="TEXT" not_null="true" foreign="commodities(id)" foreign_key_name="fk_commodity_service_commodity" on_delete="CASCADE"
	...
	//ptah:schema:field name="provider_name" type="TEXT" not_null="true"
	ProviderName string `json:"provider_name" db:"provider_name"`

	// ProviderContact is free-form (phone / email / address). No
	// validation — for the user's own reference.
	//ptah:schema:field name="provider_contact" type="TEXT"
	...
	//ptah:schema:field name="sent_at" type="TEXT" not_null="true"
	...
	//ptah:schema:field name="expected_return_at" type="TEXT"
	...
	// field alone.
	//ptah:schema:field name="returned_at" type="TEXT"
	...
	//ptah:schema:field name="cost_amount" type="DECIMAL(14,2)"
	...
	//ptah:schema:field name="cost_currency" type="TEXT"
	...
	//ptah:schema:field name="reminder_sent_overdue" type="BOOLEAN" not_null="true" default="false"
	ReminderSentOverdue bool `json:"reminder_sent_overdue" db:"reminder_sent_overdue" userinput:"false"`

	//ptah:schema:field name="reminder_sent_due_soon" type="BOOLEAN" not_null="true" default="false"
...
type CommodityServiceIndexes struct {
	...
	//ptah:schema:index name="idx_commodity_services_uuid" fields="uuid" unique="true" table="commodity_services"
	_ int

	// Index for tenant-based queries.
	//ptah:schema:index name="idx_commodity_services_tenant_id" fields="tenant_id" table="commodity_services"
	_ int

	// Composite index for tenant+group RLS-filtered queries.
	//ptah:schema:index name="idx_commodity_services_tenant_group" fields="tenant_id,group_id" table="commodity_services"
	_ int

	// Composite index for per-commodity history reads (the Service tab).
	//ptah:schema:index name="idx_commodity_services_commodity" fields="commodity_id,sent_at" table="commodity_services"
	_ int

	// Partial index for the "currently in service" group view — only
	// open rows. Cuts the index size to the working set.
	//ptah:schema:index name="idx_commodity_services_active" fields="group_id,expected_return_at" condition="returned_at IS NULL" table="commodity_services"
	_ int

	// Partial index for the reminder worker's overdue scan — open
	// services with an expected return in the past.
	//ptah:schema:index name="idx_commodity_services_due" fields="expected_return_at" condition="returned_at IS NULL AND expected_return_at IS NOT NULL" table="commodity_services"
	_ int
}
