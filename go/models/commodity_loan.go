...
//
//ptah:schema:rls:enable table="commodity_loans" comment="Enable RLS for multi-tenant commodity loan isolation"
...
//ptah:schema:rls:policy name="commodity_loan_background_worker_access" table="commodity_loans" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows background workers to access all commodity loans for processing"
//ptah:schema:table name="commodity_loans"
type CommodityLoan struct {
	...
	//ptah:schema:field name="commodity_id" type="TEXT" not_null="true" foreign="commodities(id)" foreign_key_name="fk_commodity_loan_commodity" on_delete="CASCADE"
	...
	//ptah:schema:field name="borrower_name" type="TEXT" not_null="true"
	...
	//ptah:schema:field name="borrower_contact" type="TEXT"
	...
	//ptah:schema:field name="borrower_note" type="TEXT"
	...
	//ptah:schema:field name="lent_at" type="TEXT" not_null="true"
	...
	//ptah:schema:field name="due_back_at" type="TEXT"
...
type CommodityLoanIndexes struct {
	...
	//ptah:schema:index name="idx_commodity_loans_uuid" fields="uuid" unique="true" table="commodity_loans"
	_ int

	// Index for tenant-based queries.
	//ptah:schema:index name="idx_commodity_loans_tenant_id" fields="tenant_id" table="commodity_loans"
	_ int

	// Composite index for tenant+group RLS-filtered queries.
	//ptah:schema:index name="idx_commodity_loans_tenant_group" fields="tenant_id,group_id" table="commodity_loans"
	_ int

	// Composite index for per-commodity history reads (the Lend tab).
	//ptah:schema:index name="idx_commodity_loans_commodity" fields="commodity_id,lent_at" table="commodity_loans"
	_ int
	...
	//ptah:schema:index name="idx_commodity_loans_active" fields="group_id,due_back_at" condition="returned_at IS NULL" table="commodity_loans"
	_ int

	// Partial index for the reminder worker's overdue scan — open
	// loans with a due date in the past.
	//ptah:schema:index name="idx_commodity_loans_due" fields="due_back_at" condition="returned_at IS NULL AND due_back_at IS NOT NULL" table="commodity_loans"
	_ int
}
