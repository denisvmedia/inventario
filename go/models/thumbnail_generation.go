...
//ptah:schema:rls:enable table="thumbnail_generation_jobs" comment="Enable RLS for multi-tenant thumbnail generation job isolation"
...
//ptah:schema:rls:policy name="thumbnail_generation_job_background_worker_access" table="thumbnail_generation_jobs" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows background workers to access all thumbnail generation jobs for processing"

//ptah:schema:table name="thumbnail_generation_jobs"
type ThumbnailGenerationJob struct {
	...
	//ptah:schema:field name="file_id" type="TEXT" not_null="true" foreign="files(id)" foreign_key_name="fk_thumbnail_job_file"
	...
	//ptah:schema:field name="attempt_count" type="INTEGER" not_null="true" default="0"
	AttemptCount int `json:"attempt_count" db:"attempt_count"`

	// MaxAttempts defines the maximum number of attempts before marking as failed
	//ptah:schema:field name="max_attempts" type="INTEGER" not_null="true" default="3"
	...
	//ptah:schema:field name="processing_started_at" type="TIMESTAMP"
	...
	//ptah:schema:field name="processing_completed_at" type="TIMESTAMP"
...
type ThumbnailGenerationJobIndexes struct {
	...
	//ptah:schema:index name="idx_thumbnail_jobs_uuid" fields="uuid" unique="true" table="thumbnail_generation_jobs"
	_ int

	// Index for tenant-based queries
	//ptah:schema:index name="idx_thumbnail_jobs_tenant_id" fields="tenant_id" table="thumbnail_generation_jobs"
	_ int

	// Composite index for status queries (for job processing)
	// `fields` holds column names only -- a sort direction in there is read as
	// part of the name and quoted with it. ASC is the default the live index
	// already has, so it is simply omitted.
	//ptah:schema:index name="idx_thumbnail_jobs_status_created" fields="status,created_at" table="thumbnail_generation_jobs"
	_ int

	// Index for file-based queries
	//ptah:schema:index name="idx_thumbnail_jobs_file_id" fields="file_id" table="thumbnail_generation_jobs"
	_ int

	// Composite index for user and status queries
	//ptah:schema:index name="idx_thumbnail_jobs_user_status" fields="user_id,status" table="thumbnail_generation_jobs"
	_ int

	// Index for cleanup queries (completed/failed jobs)
	//ptah:schema:index name="idx_thumbnail_jobs_cleanup" fields="status,processing_completed_at" table="thumbnail_generation_jobs"
	_ int
}
...
// Enable RLS for multi-tenant isolation
//ptah:schema:rls:enable table="user_concurrency_slots" comment="Enable RLS for multi-tenant user concurrency slot isolation"
...
//ptah:schema:rls:policy name="user_concurrency_slot_background_worker_access" table="user_concurrency_slots" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows background workers to access all user concurrency slots for coordination"

//ptah:schema:table name="user_concurrency_slots"
type UserConcurrencySlot struct {
	//ptah:embedded mode="inline"
	TenantUserAwareEntityID

	// JobID is the ID of the thumbnail generation job currently using this slot
	//ptah:schema:field name="job_id" type="TEXT" not_null="true" foreign="thumbnail_generation_jobs(id)" foreign_key_name="fk_concurrency_slot_job"
...
type UserConcurrencySlotIndexes struct {
	// Index for tenant-based queries
	//ptah:schema:index name="idx_user_concurrency_slots_tenant_id" fields="tenant_id" table="user_concurrency_slots"
	_ int

	// Index for user-based queries
	//ptah:schema:index name="idx_user_concurrency_slots_user_id" fields="user_id" table="user_concurrency_slots"
	_ int

	// Unique index for job-based queries (one slot per job)
	//ptah:schema:index name="idx_user_concurrency_slots_job_id" fields="job_id" unique="true" table="user_concurrency_slots"
	_ int

	// Index for status-based queries
	//ptah:schema:index name="idx_user_concurrency_slots_status" fields="status" table="user_concurrency_slots"
	_ int

	// Composite index for user and status queries
	//ptah:schema:index name="idx_user_concurrency_slots_user_status" fields="user_id,status" table="user_concurrency_slots"
	_ int
}
