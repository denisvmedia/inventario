...
//ptah:schema:rls:enable table="user_mfa_secrets" comment="Enable RLS for per-user MFA secret isolation"
...
//ptah:schema:rls:policy name="user_mfa_background_worker_access" table="user_mfa_secrets" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows the login flow + management endpoints to read the row before RLS context is established on the connection"

//ptah:schema:table name="user_mfa_secrets"
type UserMFASecret struct {
	...
	// verification.
	//ptah:schema:field name="secret_encrypted" type="TEXT" not_null="true"
	...
	//ptah:schema:field name="enabled_at" type="TIMESTAMP"
	...
	//ptah:schema:field name="backup_codes_hashed" type="JSONB" not_null="true" default="'[]'"
	...
	//ptah:schema:field name="last_used_step" type="BIGINT" not_null="true" default="0"
...
type UserMFASecretIndexes struct {
	...
	//ptah:schema:index name="idx_user_mfa_secrets_uuid" fields="uuid" unique="true" table="user_mfa_secrets"
	_ int

	// One MFA row per user within a tenant.
	//ptah:schema:index name="idx_user_mfa_secrets_user" fields="tenant_id,user_id" unique="true" table="user_mfa_secrets"
	_ int
}
