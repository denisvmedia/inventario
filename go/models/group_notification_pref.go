...
//
//ptah:schema:rls:enable table="group_notification_prefs" comment="Enable RLS for multi-tenant per-group notification prefs isolation"
...
//ptah:schema:rls:policy name="group_notification_prefs_background_worker_access" table="group_notification_prefs" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows background workers to read per-group prefs when deciding whether to enqueue a reminder"
...
//
//ptah:schema:table name="group_notification_prefs"
type GroupNotificationPref struct {
	//ptah:embedded mode="inline"
	TenantAwareEntityID

	// GroupID references the location group this pref applies to.
	//ptah:schema:field name="group_id" type="TEXT" not_null="true" foreign="location_groups(id)" foreign_key_name="fk_group_notif_pref_group"
	...
	//ptah:schema:field name="user_id" type="TEXT" not_null="true" foreign="users(id)" foreign_key_name="fk_group_notif_pref_user"
	...
	//ptah:schema:field name="enabled" type="BOOLEAN" not_null="true"
...
type GroupNotificationPrefIndexes struct {
	...
	// import/restore).
	//ptah:schema:index name="idx_group_notification_prefs_uuid" fields="uuid" unique="true" table="group_notification_prefs"
	_ int

	// Upsert key: one row per (user, group, category).
	//ptah:schema:index name="idx_group_notification_prefs_unique" fields="tenant_id,group_id,user_id,category" unique="true" table="group_notification_prefs"
	_ int

	// "All prefs for this user inside this group" — the GET endpoint
	// hits this on every settings-page open.
	//ptah:schema:index name="idx_group_notification_prefs_user_group" fields="user_id,group_id" table="group_notification_prefs"
	_ int

	//ptah:schema:index name="idx_group_notification_prefs_tenant_id" fields="tenant_id" table="group_notification_prefs"
	_ int
}
