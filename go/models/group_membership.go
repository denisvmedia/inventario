...
//
//ptah:schema:rls:enable table="group_memberships" comment="Enable RLS for multi-tenant group membership isolation"
...
//ptah:schema:rls:policy name="group_membership_background_worker_access" table="group_memberships" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows background workers to access all group memberships for processing"

//ptah:schema:table name="group_memberships"
type GroupMembership struct {
	//ptah:embedded mode="inline"
	TenantAwareEntityID

	// GroupID references the location group this membership belongs to.
	//ptah:schema:field name="group_id" type="TEXT" not_null="true" foreign="location_groups(id)" foreign_key_name="fk_membership_group"
	...
	//ptah:schema:field name="member_user_id" type="TEXT" not_null="true" foreign="users(id)" foreign_key_name="fk_membership_user"
	...
	//ptah:schema:field name="joined_at" type="TIMESTAMP" not_null="true" default_expr="CURRENT_TIMESTAMP"
...
type GroupMembershipIndexes struct {
	...
	//ptah:schema:index name="idx_group_memberships_uuid" fields="uuid" unique="true" table="group_memberships"
	_ int

	// Unique index ensuring a user can only have one membership per group within a tenant
	//ptah:schema:index name="idx_group_memberships_unique" fields="tenant_id,group_id,member_user_id" unique="true" table="group_memberships"
	_ int

	// Index for listing members of a group
	//ptah:schema:index name="idx_group_memberships_group_id" fields="group_id" table="group_memberships"
	_ int

	// Index for listing groups a user belongs to
	//ptah:schema:index name="idx_group_memberships_member_user_id" fields="member_user_id" table="group_memberships"
	_ int

	// Index for tenant-based queries
	//ptah:schema:index name="idx_group_memberships_tenant_id" fields="tenant_id" table="group_memberships"
	_ int
}
