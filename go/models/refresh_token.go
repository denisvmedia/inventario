...
//ptah:schema:rls:enable table="refresh_tokens" comment="Enable RLS for multi-tenant refresh token isolation"
...
//ptah:schema:rls:policy name="refresh_token_background_worker_access" table="refresh_tokens" for="ALL" to="inventario_background_worker" using="true" with_check="true" comment="Allows background workers to access all refresh tokens for cleanup"

//ptah:schema:table name="refresh_tokens"
...
type RefreshTokenIndexes struct {
	...
	//ptah:schema:index name="idx_refresh_tokens_uuid" fields="uuid" unique="true" table="refresh_tokens"
	_ int

	// Index for user-based queries
	//ptah:schema:index name="idx_refresh_tokens_user_id" fields="user_id" table="refresh_tokens"
	_ int

	// Unique index for token hash lookups
	//ptah:schema:index name="idx_refresh_tokens_token_hash" fields="token_hash" unique="true" table="refresh_tokens"
	_ int

	// Index for expiry-based cleanup
	//ptah:schema:index name="idx_refresh_tokens_expires_at" fields="expires_at" table="refresh_tokens"
	_ int
}
