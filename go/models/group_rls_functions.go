...
// GroupRLSFunctions carries the group context functions for Row-Level Security.
//
...
//
//ptah:schema:function name="set_group_context" params="group_id_param TEXT" returns="VOID" language="plpgsql" body="BEGIN PERFORM set_config('app.current_group_id', group_id_param, true); END;" comment="Sets the current group context for RLS policies (transaction-local)"
//ptah:schema:function name="get_current_group_id" returns="TEXT" language="plpgsql" volatility="STABLE" body="BEGIN RETURN current_setting('app.current_group_id', true); END;" comment="Gets the current group ID from session for RLS policies"
