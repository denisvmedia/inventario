...
func (murd MaintenanceScheduleUpdateRequestData) ValidateWithContext(ctx context.Context) error {
	...
	if murd.Title != nil {
		fields = append(fields, validation.Field(&murd.Title, validation.Required, validation.Length(1, 200)))
	}
	if murd.IntervalDays != nil {
		fields = append(fields, validation.Field(&murd.IntervalDays, validation.Required, validation.Min(1), validation.Max(36500)))
	}
	if murd.Notes != nil {
