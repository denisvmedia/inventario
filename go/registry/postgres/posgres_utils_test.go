...
func setupTestTenantAndUser(c *qt.C, registrySet *registry.Set) {
	...
	testUser := models.User{
		TenantAwareEntityID: models.TenantAwareEntityID{
			// ID will be generated by the Create method
			TenantID: "test-tenant-id",
			// UserID will be set to self-reference by the Create method
		},
	...
	if existingUser == nil {
		...
		createdUser, err := registrySet.UserRegistry.Create(ctx, testUser)
		c.Assert(err, qt.IsNil)
		c.Assert(createdUser, qt.IsNotNil)
		c.Assert(createdUser.ID, qt.Not(qt.Equals), "")
		c.Assert(createdUser.UserID, qt.Equals, createdUser.ID) // Should be self-reference
	}
}
