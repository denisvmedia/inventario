...
// The race-safety guarantee that used to live here moved to the
// SystemAdminGrantRegistry tests (#1784): the system-admin privilege
// is no longer a column on the users row, so the test that pins the
// concurrent-revoke invariant moved alongside the new registry — see
// TestSystemAdminGrantRegistry_RevokeAtomic_NoConcurrentLastAdminRevoke
// in system_admin_grants_test.go.
