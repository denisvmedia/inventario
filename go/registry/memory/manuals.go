...
func (r *ManualRegistry) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return errxtrace.Wrap("failed to get manual", err)
	}
	...
	if err != nil {
		return errxtrace.Wrap("failed to delete manual", err)
	}

	err = r.commodityRegistry.DeleteManual(ctx, manual.CommodityID, id)
	if err != nil {
		return errxtrace.Wrap("failed to delete manual from commodity", err)
	}
