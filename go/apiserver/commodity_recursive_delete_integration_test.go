...
func TestCommodityDeleteRecursive_Integration(t *testing.T) {
	...
	createdImageFile, err := getRegistrySetFromParams(params, testUser).FileRegistry.Create(ctx, imageFile)
	c.Assert(err, qt.IsNil)
	...
	createdManualFile, err := getRegistrySetFromParams(params, testUser).FileRegistry.Create(ctx, manualFile)
	c.Assert(err, qt.IsNil)
	...
	createdInvoiceFile, err := getRegistrySetFromParams(params, testUser).FileRegistry.Create(ctx, invoiceFile)
	c.Assert(err, qt.IsNil)

	// Verify all files exist and are linked to the commodity
	files, err := getRegistrySetFromParams(params, testUser).FileRegistry.ListByLinkedEntity(ctx, "commodity", createdCommodity.ID)
	c.Assert(err, qt.IsNil)
	c.Assert(files, qt.HasLen, 3)

	// Verify each file type exists
	imageFiles, err := getRegistrySetFromParams(params, testUser).FileRegistry.ListByLinkedEntityAndMeta(ctx, "commodity", createdCommodity.ID, "images")
	c.Assert(err, qt.IsNil)
	c.Assert(imageFiles, qt.HasLen, 1)

	manualFiles, err := getRegistrySetFromParams(params, testUser).FileRegistry.ListByLinkedEntityAndMeta(ctx, "commodity", createdCommodity.ID, "manuals")
	c.Assert(err, qt.IsNil)
	c.Assert(manualFiles, qt.HasLen, 1)

	invoiceFiles, err := getRegistrySetFromParams(params, testUser).FileRegistry.ListByLinkedEntityAndMeta(ctx, "commodity", createdCommodity.ID, "invoices")
	c.Assert(err, qt.IsNil)
	...
	_, err = getRegistrySetFromParams(params, testUser).FileRegistry.Get(ctx, createdImageFile.ID)
	c.Assert(err, qt.IsNotNil) // Should be deleted

	_, err = getRegistrySetFromParams(params, testUser).FileRegistry.Get(ctx, createdManualFile.ID)
	c.Assert(err, qt.IsNotNil) // Should be deleted

	_, err = getRegistrySetFromParams(params, testUser).FileRegistry.Get(ctx, createdInvoiceFile.ID)
	c.Assert(err, qt.IsNotNil) // Should be deleted

	// Verify area still exists (should not be affected)
	_, err = getRegistrySetFromParams(params, testUser).AreaRegistry.Get(ctx, areas[0].ID)
