...
func (api *filesAPI) createFile(w http.ResponseWriter, r *http.Request) {
	...
	tagSlugs, terr := api.tagService.NormalizeAndEnsureSlugs(r.Context(), models.TagKindFile, input.Data.Attributes.Tags)
	if terr != nil {
...
func (api *filesAPI) streamFileFromStorage(w http.ResponseWriter, r *http.Request, filePath, mimeType, filename string, disposition services.Disposition) {
	...
	if err != nil {
		internalServerError(w, r, errxtrace.Wrap("failed to check file existence", err))
		return
	}
	if !exists {
		renderEntityError(w, r, ErrNotFound)
	...
	if err != nil {
		internalServerError(w, r, errxtrace.Wrap("failed to get file attributes", err))
		return
	}
	...
	if err != nil {
		internalServerError(w, r, errxtrace.Wrap("failed to open file reader", err))
		return
	}
	defer reader.Close()
