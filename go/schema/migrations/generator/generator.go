...
func (m *Generator) GenerateMigrationFiles(ctx context.Context, migrationName, migrationsDir string) (*generator.MigrationFiles, error) {
	...
	if err != nil {
		return nil, stacktrace.Wrap("failed to create output directory", err)
	}

	// Connect to database first
	conn, err := dbschema.ConnectToDatabase(m.dbURL)
	if err != nil {
		return nil, stacktrace.Wrap("failed to connect to database", err)
	}
	defer func() {
		err := conn.Close()
		if err != nil {
	...
	if err != nil {
		return nil, stacktrace.Wrap("failed to generate migration files", err)
	}
...
func (m *Generator) GenerateSchemaSQL(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, stacktrace.Wrap("failed to parse Go schema", err)
	}
